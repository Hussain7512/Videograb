...
class VideoGrabApp(App):
    def build(self):
        ...
        btn = Button(text="Download", size_hint=(1, 0.15), background_color=(0, 0.7, 1, 1))
        btn.bind(on_press=self.start_download)
        layout.add_widget(btn)
        
        self.status = Label(text="Taiyar hai", size_hint=(1, 0.3))
    ...
    def download_video(self, link, choice):
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ydl.download([link])
            self.status.text = "✅ Download mukammal! Download folder mein hai"
        except Exception as e:
            self.status.text = f"❌ Masla: {e}"
